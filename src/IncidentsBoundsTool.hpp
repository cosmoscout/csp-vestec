...
/// Measures the area and volume of an arbitrary polygon on surface with a
/// Delaunay-mesh. It displays the bounding box of the selected polygon, which
/// can be copied for cache generator.
class IncidentsBoundsTool : public IVistaOpenGLDraw,
                            public cs::core::tools::MultiPointTool {
public:
  ...
  /// A derived class may call this in order to add a new point at the given
  /// position. If no position is given, the current pointer position will be
  /// used.
...
private:
  ...
  /// Returns the interpolated position in cartesian coordinates. The fourth
  /// component is height above the surface
  glm::dvec4 getInterpolatedPosBetweenTwoMarks(cs::core::tools::Mark const &l0,
                                               cs::core::tools::Mark const &l1,
                                               double value);
