...
TextureRenderNode::TextureRenderNode(
    ...
    : VNE::Node(pItem, id, 2, 0), m_pAnchor(pAnchor) {
...
std::string TextureRenderNode::GetName() { return "TextureRenderNode"; }
...
void TextureRenderNode::Init(VNE::NodeEditor *pEditor) {
  ...
  pEditor->GetGuiItem()->registerCallback(
      "TextureRenderNode.getNumberOfTextureLayers",
      ...
      std::function([pEditor](double id, std::string filePath) {
        pEditor->GetNode<TextureRenderNode>(std::lround(id))
            ->GetNumberOfTextureLayers(filePath);
      }));

  pEditor->GetGuiItem()->registerCallback(
      "TextureRenderNode.setTextureLayer",
  ...
  pEditor->GetGuiItem()->registerCallback(
      "TextureRenderNode.setMinMaxDataRange",
      "Sets the texture layer to be visualized",
      std::function([pEditor](double id, std::string filePath) {
        std::cout << "Filepath: " << filePath << "\n";
        pEditor->GetNode<TextureRenderNode>(std::lround(id))
            ->SetMinMaxDataRange(filePath);
      }));
  ...
  pEditor->GetGuiItem()->registerCallback<double, std::string>(
      "TextureRenderNode.setTransferFunction",
  ...
  pEditor->GetGuiItem()->registerCallback<double, bool>(
      "TextureRenderNode.setEnableTime", "Enables the simulation time",
      std::function([pEditor](double id, bool val) {
        pEditor->GetNode<TextureRenderNode>(std::lround(id))->SetUseTime(val);
      }));

  pEditor->GetGuiItem()->registerCallback<double, double>(
      "TextureRenderNode.setMipMapLevel",
      "Sets the current displayed mip map level",
      std::function([pEditor](double id, double lod) {
        pEditor->GetNode<TextureRenderNode>(std::lround(id))
            ->SetMipMapLevel(lod);
      }));

  pEditor->GetGuiItem()->registerCallback<double, bool>(
      "TextureRenderNode.setEnableManualMipMap",
      "Enables setting the mip map level manually",
      std::function([pEditor](double id, bool enable) {
        pEditor->GetNode<TextureRenderNode>(std::lround(id))
            ->EnableManualMipMaps(enable);
      }));

  pEditor->GetGuiItem()->registerCallback<double, double>(
      "TextureRenderNode.setMipMapReduceMode",
  ...
  pEditor->GetGuiItem()->registerCallback<double>(
      "TextureRenderNode.unloadTexture",
      "Unloads the currently active texture. Called by the texture node "
      "automatically",
...
void TextureRenderNode::GetNumberOfTextureLayers(std::string filePath) {
  ...
  m_pItem->callJavascript("TextureRenderNode.setNumberOfTextureLayers", GetID(),
                          bands);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void TextureRenderNode::SetTextureLayerID(int layerID) { m_iLayerID = layerID; }

////////////////////////////////////////////////////////////////////////////////////////////////////

void TextureRenderNode::SetOpacity(float val) { m_pRenderer->SetOpacity(val); }
...
void TextureRenderNode::SetTime(float val) { m_pRenderer->SetTime(val); }

////////////////////////////////////////////////////////////////////////////////////////////////////

void TextureRenderNode::SetUseTime(bool use) { m_pRenderer->SetUseTime(use); }
...
void TextureRenderNode::UnloadTexture() { m_pRenderer->UnloadTexture(); }
...
void TextureRenderNode::ReadSimulationResult(std::string filename) {
  ...
  m_pItem->callJavascript("TextureRenderNode.setMipMapLevels", GetID(),
                          m_pRenderer->GetMipMapLevels());
}