...
std::string CriticalPointsNode::GetName() { return "CriticalPointsNode"; }
...
void CriticalPointsNode::Init(VNE::NodeEditor *pEditor) {
  ...
  std::string code = cs::utils::filesystem::loadToString(
      "../share/resources/gui/js/csp-vestec-cp-render-node.js");
  ...
  pEditor->GetGuiItem()->registerCallback<double, std::string>(
      "setPoints", "Reads simulation data",
  ...
  pEditor->GetGuiItem()->registerCallback<double, double>(
      "setOpacity", "Adjust the opacity of the rendering",
  ...
  pEditor->GetGuiItem()->registerCallback<double, std::string>(
      "CriticalPointsNode.setTransferFunction",
  ...
  pEditor->GetGuiItem()->registerCallback<double, double>(
      "setCriticalPointsVisualizationMode", "Sets the visualization mode",
      std::function([pEditor](double id, double val) {
        CriticalPointsRenderer::RenderMode renderMode =
            CriticalPointsRenderer::RenderMode::ALL;
...
void CriticalPointsNode::SetOpacity(float val) { m_pRenderer->SetOpacity(val); }
...
void CriticalPointsNode::SetPoints(const std::string &jsonObj) {
  ...
  for (auto &element : args) {
    ...
    glm::dvec3 posU(element["coordinates"]["upper"]["x"],
                    element["coordinates"]["upper"]["y"],
                    element["coordinates"]["upper"]["z"]);
    glm::dvec3 posL(element["coordinates"]["lower"]["x"],
                    element["coordinates"]["lower"]["y"],
  ...
  m_pItem->callJavascript("CriticalPointsNode.setRange", GetID(),
  ...
  csp::vestec::logger().debug("[{}::SetPoints] Got points from JavaScript",
                              GetName());
}
