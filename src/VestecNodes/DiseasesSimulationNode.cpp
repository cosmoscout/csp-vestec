...
std::string DiseasesSimulation::GetName() { return "DiseasesSimulation"; }
...
void DiseasesSimulation::Init(VNE::NodeEditor *pEditor) {
  ...
  if (!csp::vestec::Plugin::vestecDiseasesDir.empty()) {
    pEditor->GetGuiItem()->callJavascript(
        "DiseasesSimulationNode.setPath",
        csp::vestec::Plugin::vestecDiseasesDir);
  }
  ...
  pEditor->GetGuiItem()->registerCallback(
      "DiseasesSimulationNode.readDiseasesSimulationModes",
...
void DiseasesSimulation::GetFileNamesForTimeStep(int id,
  ...
  m_pItem->callJavascript("DiseasesSimulationNode.setFileListForTimeStep", id,
                          args.dump());
}

void DiseasesSimulation::SetNumberOfEnsembleMembers(int id,
  ...
  m_pItem->callJavascript("DiseasesSimulationNode.setNumberOfEnsembleMembers",
                          id, lDirs.size(), lFiles.size());
}
...
void DiseasesSimulation::SetSimulationModes(int id, const std::string &path) {
  ...
  m_pItem->callJavascript("DiseasesSimulationNode.fillSimModes", id,
                          args.dump());
}
