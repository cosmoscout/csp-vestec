...
class UncertaintyRenderNode {
  ...
  builder(node) {
    ...
    const opacityControl = new D3NE.Control(
        ...
        <div class="row">
          ...
          <select id="uncertainty-node_${
            node.id}-vis_mode" class="combobox col-7">
        ...
        (element, _control) => {
          const slider = element.querySelector(
              `#uncertainty-node_${node.id}-slider_opacity`);
          noUiSlider.create(
          ...
          slider.noUiSlider.on('slide', (values, handle) => {
            window.callNative('UncertaintyRenderNode.setOpacityUncertainty',
                              node.id, parseFloat(values[handle]));
          });

          // Initialize combobox for the visualization mode
          const select =
              $(element).find(`#uncertainty-node_${node.id}-vis_mode`);
          select.selectpicker();
    ...
    const inputTransferFunction =
        new D3NE.Input('Transfer Function (average)',
                       CosmoScout.vestecNE.sockets.TRANSFER_FUNCTION);
    node.addInput(inputTransferFunction);
    const inputTransferFunctionUncertainty =
        new D3NE.Input('Transfer Function (variance, difference)',
                       CosmoScout.vestecNE.sockets.TRANSFER_FUNCTION);
  ...
  _checkTextureInput(node, textureInput) {
    ...
    window.callNative('UncertaintyRenderNode.setTextureFiles', node.id,
                      textures);
...
(() => {
  ...
  CosmoScout.vestecNE.addNode('UncertaintyRenderNode',
                              renderNode.getComponent());
})();
